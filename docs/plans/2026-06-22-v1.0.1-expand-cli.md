# v1.0.1 Expand CLI Implementation Plan
...
- **Config schema_version:** stays `"1"` for v1.0.x (decision Q18=C — no breaking changes)
...
- **No streaming in this slice** (Q11=B1, streaming deferred to v1.0.6)
- **No fallback wiring** (Q10=A, deferred to v1.0.4)
- **No timeout** (Q12=C, deferred to v1.0.5)
- **Zhurongshuo-style prompt injection is NOT in this slice** (Q16=C, deferred to v1.0.2 — this slice ships with existing placeholder strings)
...
| `docs/PROJECT_STATUS.md` | Modify | TL;DR clarify v1.0.0 scope; §11 tech debt list audit (some items already fixed); §10 待做 list reorder |
| `docs/ROADMAP.md` | Modify | Add "v1.0.0 premature" preamble; reorder v1.0.x slices per Q21=A |
| `EXTRACTION_NOTES.md` | Modify | Remove "待审阅" label (validated by 7 SDD reviews) |
| `docs/decisions/26-grill-decisions.md` | Modify | Append v1.0.x grill (Q1-Q21) decisions section |
...
**Why first:** All 21 grill decisions need to be persisted before any code work, so a future agent reading `decisions/` understands why v1.0.1 is shaped the way it is. ROADMAP/PROJECT_STATUS need reordering before code lands so commit messages can reference "v1.0.1 = first of revised v1.0.x series".
...
- Modify: `docs/ROADMAP.md:1-78` (preamble + v1.0.x reorder)
...
- [ ] **Step 0.1: Append v1.0.x grill decisions to decisions doc**
...
> 以下 21 项决策对齐 v1.0.x 切片的实施边界。

### 范围重定

| # | 维度 | 决定 | 影响 |
|---|---|---|---|
| Q1 | v1.0.0 完成标准 | 用户能从 CLI 跑出至少一章节 = ship 门槛；当前 tag 过早 | v1.0.x 系列补齐 |
...
| Q8 | export 目标 | 只 md 单文件（v1.0.2） | zhurongshuo/hugo/pdf 推 v1.1.4 |
...
In `README.md`:
- Line 7-9: Change `**v1.0.0 — S7 (Expand complete)**` to `**v1.0.1 (in progress)**`
- Line 9: Change `See \`docs/superpowers/plans/\` for roadmap.` to `See \`docs/ROADMAP.md\` for the v1.0.x roadmap.`
...
- **质量缺口**：expand prompt 注入全是占位符（v1.0.2 修）
...
- `book.Citation.UsedInParagraph` (book/types.go:93) — **still exists, never populated** (推 v1.1 design decision)
- `expand.types.ExpandOutput.Draft` — **still exists, kept for debug** (推 v1.1 design decision)
...
- `expand.types.ExpandOutput.Draft` 字段保留 pre-validation draft 用于 debug（v1.1 决定是否暴露为 CLI debug flag）
- `cli.chatterProviderHook` 是 test-only 全局可变 var（v1.1 重构为 struct field）
...
Reorder the v1.0.x list to match Q21=A:

```markdown
### v1.0.x（让 v1.0 真正名副其实）
- [ ] `jianwu expand <slug> <NN-MM>` CLI 命令（**v1.0.1**，进行中）
- [ ] Expand prompt 注入 archetype + samples + adjacent + similar book（**v1.0.2**，从 v1.1.6 上移）
- [ ] `jianwu review/finalize/export/status` 命令（**v1.0.3**）
- [ ] Fallback model wiring（**v1.0.4**）
- [ ] LLM 超时（**v1.0.5**）
- [ ] Streaming 输出（**v1.0.6**，可选）

**v1.0.5 ship 后视为 v1.0 promise fully delivered.**
```

- [ ] **Step 0.6: Update ROADMAP.md with v1.0.0 premature preamble**
...
- [ ] **Step 0.7: Update ROADMAP.md v1.0.x slices**

Replace the existing v1.0.1-v1.0.5 section with the reordered one. The new v1.0.2 (Expand Prompt 注入) should be inserted between v1.0.1 (expand CLI) and the old v1.0.2 (now v1.0.3, status commands). Remove the v1.1.6 entry from v1.1 section since it's been promoted to v1.0.2.

The final v1.0.x section should have these slices in order:
- v1.0.1 — Expand CLI (current, expanded content from old v1.0.1)
- v1.0.2 — Expand Prompt 注入 (NEW, content moved from v1.1.6)
- v1.0.3 — Review + Finalize + Export + Status (was v1.0.2)
- v1.0.4 — Fallback Model Wiring (was v1.0.3)
- v1.0.5 — LLM Timeout + Cancellation (was v1.0.5; v1.0.4 Streaming pushed to v1.0.6)
- v1.0.6 — Streaming Output (was v1.0.4, demoted to optional)
...
- Line 3: change `> 本文档记录 v1.0 前置工作（DESIGN.md 第 13 节）的执行情况，` (keep) but verify line 9 status
- Line 9: change `状态：**v1.0.0 ship 时已通过 7 个切片的 SDD review 验证**。低信心（标记 medium）的 corpus abstract 仍可后续修订（v1.1 \`corpus sync\` 时）。` — this is already accurate, no change needed
...
git commit -m "docs: append 21 v1.0.x audit decisions to grill decisions archive"

git add docs/PROJECT_STATUS.md
git commit -m "docs: PROJECT_STATUS — clarify v1.0.0 premature, audit §11 tech debt, reorder §10"

git add docs/ROADMAP.md
git commit -m "docs: ROADMAP — add v1.0.0 premature preamble, reorder v1.0.x slices"
...
**Why:** The expand command needs to assemble chatter + searcher + reader + embedder as a bundle. Decision Q20=B says use a single `providerDepsHook` returning a struct, which pre-scripts the v1.1 refactor of `chatterProviderHook`. The existing `chatterProviderHook` is untouched (Q14=C).
...
Per decision Q20=B, single struct prefigures v1.1 refactor of
...
**Why:** expand CLI needs to write `chapters/NN-MM.md` with YAML frontmatter per Q2=B schema, and read it back for the `--force` check (refuse overwrite of `reviewed`/`final`). Putting this in `internal/book/` makes it reusable for future v1.0.3 commands.
...
Reused by v1.0.1 expand CLI and v1.0.3 status commands."
```
...
**Why:** expand CLI needs to assemble a `*expand.ToolRegistry` from `ProviderDeps` + outline (for the adjacent-chapter callback). The outline callback is a stub in v1.0.1 (returns empty string) — real implementation is v1.0.2 per Q16=C.
...
v1.0.1 outlineFn is a stub; v1.0.2 will inject real adjacent chapter
...
before tagging v1.0.1 to verify real Provider chain works."
```
...
**Why:** Before tagging v1.0.1, run the standard checks: go vet, gofmt, all tests, manual smoke test.
...
- [ ] **Step 8.7: Update PROJECT_STATUS.md to mark v1.0.1 complete**

In `docs/PROJECT_STATUS.md`:
- §9 (已交付): Add v1.0.1 entry under CLI commands
- §10 (待做): Mark v1.0.1 done, push cursor to v1.0.2

Commit:
```bash
git add docs/PROJECT_STATUS.md
git commit -m "docs: PROJECT_STATUS — mark v1.0.1 expand CLI shipped"
```

- [ ] **Step 8.8: Tag v1.0.1**

```bash
git tag -a v1.0.1 -m "v1.0.1: expand CLI

First slice of v1.0.x series (per Q1=B, Q17=A).
...
- zhurongshuo-style prompt injection (v1.0.2, Q16=C)
- review/finalize/export/status commands (v1.0.3)
- fallback model wiring (v1.0.4)
- LLM timeout (v1.0.5)
- streaming output (v1.0.6, optional)

Note: v1.0.0 tag was premature. v1.0.5 ship will mark true v1.0 delivery."

# Don't push tag automatically — confirm with user first
echo "Tagged v1.0.1 locally. Run: git push origin v1.0.1"
```
...
- Q1 (v1.0.0 promise): This slice ships the CLI command (1 of 5 needed for true v1.0)
...
- Q7 (status transitions): Out of scope for v1.0.1 (v1.0.3) — correct
- Q8 (md only export): Out of scope (v1.0.3) — correct
- Q9 (dry-run for finalize/export): Out of scope (v1.0.3) — correct
- Q10 (global fallback): Out of scope (v1.0.4) — correct
- Q11 (streaming): Out of scope (v1.0.6) — correct
- Q12 (timeout): Out of scope (v1.0.5) — correct
...
- Q16 (prompt injection): Out of scope (v1.0.2) — explicitly noted in Task 3 (outlineFn stub)
- Q17 (v1.0.0 tag handling): Task 0 adds preamble to ROADMAP + PROJECT_STATUS ✓
...
- Q21 (slice order): v1.0.1 is first in the Q21=A sequence ✓

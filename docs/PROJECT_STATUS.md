...
- **当前版本**：**v0.1.6**（v0.1.x 系列全部交付，实际迭代完成度 v0.1.0–v0.1.6）
...
- **下一里程碑**：**v0.2.2 自动事实复核** — 先优化 claims 抽取精度，再加 `jianwu verify <slug>` 命令
...
- [x] Fallback model wiring（**v0.1.4**，已交付，见文档）
- [x] LLM 调用超时（**v0.1.5**，已交付，见文档）
- [x] Streaming 输出（**v0.1.6**，已交付，可选 polish）
...
`ModelRef` 内嵌 `Fallback *ModelRef`；`buildChatter` 静态装配 `FallbackWrapper`；primary==fallback 时跳过（打 warn 日志）。配置模板含 fallback 示例。单元测试 + E2E mock 测试。

### v0.1.5 LLM Timeout（2026-06-26）

`LLMConfig.TimeoutSeconds`（全局默认 90s）+ `ModelRef.TimeoutSeconds` 阶段覆盖（expand 600s）。`stageCtx()` 替代 `defaultCtx()`，每个阶段自动带超时 + Ctrl+C 信号处理。

### v0.1.6 Streaming Output（2026-06-26）

`llm.Streamer` 接口（`Stream(ctx, req) <-chan StreamChunk`），Gemini genai SDK + GLM SSE 实现。Mock 可预设 token 序列。`-v` 模式逐 token stdout。

### v0.1.x 交付总结

v0.1.0 的过早 tag 经过 6 个切片（v0.1.1–v0.1.6）补齐。v0.1.x 正式交付。
